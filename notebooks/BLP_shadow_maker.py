...
for subject in tqdm(range(50), total=50):
    ...
    for band in range(1,10):
        ...
        for i in range(band_data.shape[0]):
            ...
            dsp[:full_samp,:] = np.average(np.reshape(power[:full_samp*125*AVG],(-1,125*AVG,power.shape[1])),1)
            if extra:
    ...
    np.savez_compressed(f"/home/raffaelli/NonLinearity/NonLinearityData/EEG_el_so_BLP_NEW/NEW_electrodeBLP_fixedTime_avg{AVG:1}_shadow_bands", BLP=shadow[:,:,:,band-1])
for band in range(1,10):
    savemat(f"/home/raffaelli/NonLinearity/NonLinearityData/EEG_el_so_BLP_NEW/NEW_electrodeBLP_fixedTime_avg{AVG:1}_shadow_band{band}.mat", {"BLP":shadow[:,:,:,band-1]})