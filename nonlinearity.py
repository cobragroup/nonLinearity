...
class NonLinearEstimator:
    def __init__ (self, configFile = None):
        ...
        print(f"Using: {os.path.abspath(self.fileName)} and {os.path.abspath(self.folderName)}")
    ...
    def load_data (self):
        ...
        print("Loaded data matrix: {} samples by {} regions by {} sessions".format(self.duration, self.regions, self.sessions))
        if self.nsamples == 0:
...
# %%
    def run (self):
        ...
        self.corrector = Corrector(self.steps, self.folderName, self.iters, self.nsamples, self.nbins, self.workers, display=self.display)
    ...
    def estimate (self):
        ...
        for patientN in range(self.sessions):
            if not os.path.isfile(f"{self.folderName}/patient{patientN:02}_{self.nbins}.npy"):
                ...
                for ns, patient in tqdm(enumerate(task_producer(self.mat[:, :, patientN], self.Nsurrogates)), total=self.Nsurrogates+1, desc=f"Patient {patientN} true"):
                    statsMI[:, ns] = pool.starmap(pair_mutual_information, ((
                        patient[:, zone1], patient[:, zone2], self.nbins) for zone1 in range(self.regions) for zone2 in range(zone1+1, self.regions)))

                statsMI_shadow = np.zeros([pairNum, self.Nsurrogates+1])
                for ns, patient in tqdm(enumerate(task_producer(shadow[:, :], self.Nsurrogates)), total=self.Nsurrogates+1, desc=f"Patient {patientN} shadow"):
                ...
                corr = np.array([np.corrcoef(self.mat[:, zone1, patientN], self.mat[:, zone2, patientN])[
                                1, 0] for zone1 in range(self.regions) for zone2 in range(zone1+1, self.regions)])

                np.save(f"{self.folderName}/patient{patientN:02}_{self.nbins}", statsMI)
                np.save(f"{self.folderName}/patient{patientN:02}_{self.nbins}_sha", statsMI_shadow)
                # np.save(f"{self.folderName}/patient{patientN:02}_{self.nbins}_uni", statsMI_univar)
                np.save(f"{self.folderName}/patient{patientN:02}_{self.nbins}_cor", corr)
            else:
                statsMI = np.load(f"{self.folderName}/patient{patientN:02}_{self.nbins}.npy")
                ...
                corr = np.load(f"{self.folderName}/patient{patientN:02}_{self.nbins}_cor.npy")

            correctedperc95pointer = (self.Nsurrogates*(0.95)-0.5)/(self.Nsurrogates-1)
            correctedperc99pointer = (self.Nsurrogates*(0.99)-0.5)/(self.Nsurrogates-1)
            correctedperc01pointer = (self.Nsurrogates*(0.01)-0.5)/(self.Nsurrogates-1)
            ...
            pvalue_NMI = 1 - np.sum(statsMI[:,1:].T < statsMI[:, 0], 0)/(self.Nsurrogates+1)
            ...
            if not os.path.isfile(f"{self.folderName}/patient{patientN:02}_{self.nbins}.pdf") or self.display:
                ...
                plt.ylim(bottom=0)
                if not os.path.isfile(f"{self.folderName}/patient{patientN:02}_{self.nbins}.pdf"):
                    plt.savefig(f"{self.folderName}/patient{patientN:02}_{self.nbins}.pdf")
                if self.display:
                    plt.show()
                else:
                    plt.close()
