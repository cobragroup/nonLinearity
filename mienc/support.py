...
class returnStats (Structure):
    _fields_ = [("ratio95control", c_double), ("ratio99control", c_double), ("ratio05", c_double), ("ratio95",
                                                                                                    c_double), ("ratio99", c_double), ("totalMI", c_double), ("gaussMI", c_double), ("sigmaGaussMI", c_double)]
...
_libMI.statistics.argtypes = (POINTER(c_double), c_int, c_int, POINTER(
    c_double), POINTER(c_double), c_int, c_int, c_bool)
_libMI.statistics.restype = returnStats
_libMI.correct_vector.argtypes = (POINTER(c_double), c_int, POINTER(
    c_double), POINTER(c_double), c_int, POINTER(c_double))
_libMI.correct_vector.restype = None
_libMI.quantile_vector.argtypes = (
    POINTER(c_double), c_int, c_int, POINTER(c_double), c_int, POINTER(c_double))
...
def total_mutual_information(data, binNo=None):
    ...
    _libMI.total_mutual_information(data.ctypes.data_as(POINTER(c_double)), c_int(
        times), c_int(regions), c_int(binNo), out.ctypes.data_as(POINTER(c_double)))
    return out
...
def correct_vector(data: np.ndarray, estim: np.ndarray, actual: np.ndarray):
    ...
    _libMI.correct_vector(data.ctypes.data_as(POINTER(c_double)), c_int(totPairs), estim.ctypes.data_as(POINTER(
        c_double)), actual.ctypes.data_as(POINTER(c_double)), c_int(bins), out.ctypes.data_as(POINTER(c_double)))
    return out
...
def statistics(data: np.ndarray, estim: np.ndarray, actual: np.ndarray, numThreads: int, extended_stats: bool):
    ...
    bins = len(estim)
    tmp = _libMI.statistics(data.ctypes.data_as(POINTER(c_double)), c_int(numPairs), c_int(numSurrogatesPU-1), estim.ctypes.data_as(
        POINTER(c_double)), actual.ctypes.data_as(POINTER(c_double)), c_int(bins), c_int(numThreads), c_bool(extended_stats))
...
def quantile_vector(data: np.ndarray, quantile: Union[float, np.ndarray]):
    ...
    _libMI.quantile_vector(data.ctypes.data_as(POINTER(c_double)), c_int(numPairs), c_int(
        numSurrogatesPU-1), quantile.ctypes.data_as(POINTER(c_double)), c_int(len(quantile)), out.ctypes.data_as(POINTER(c_double)))
    return out
